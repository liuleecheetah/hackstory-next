...
/** track 沒指定顏色時輪流使用的預設色 */
const PALETTE = ['#3b6ea5', '#d97706', '#0f766e', '#9333ea', '#be123c', '#4d7c0f']

/** 依事件推算整份文件的時間範圍（毫秒） */
function eventsExtent(doc: TimelineDocument): [number, number] | null {
  let min = Infinity
  let max = -Infinity
...
/** 初始可視範圍：優先用 display.range 的建議，否則用事件的實際範圍，前後各留 3% 呼吸空間 */
function initialDomainOf(doc: TimelineDocument): [number, number] {
  let extent = eventsExtent(doc)
  const range = doc.display?.range
...
export function TimelineView({ doc, scaleRequest, onScaleModeChange }: Props) {
  const containerRef = useRef<HTMLDivElement>(null)
  const svgRef = useRef<SVGSVGElement>(null)
  const [width, setWidth] = useState(960)
  const initialDomain = useMemo(() => initialDomainOf(doc), [doc])
  ...
  const layout = useMemo(() => {
    ...
    const tracks = [...doc.tracks].sort((t1, t2) => (t1.order ?? 0) - (t2.order ?? 0))
    let y = AXIS_H + 8

    const bands = tracks.map((track, i) => {
      const color = track.color ?? PALETTE[i % PALETTE.length]
      ...
      const items = doc.events
        ...
        .flatMap((ev) => {
          ...
          return [{ ev, kind, shapeL, shapeR, label, labelW, labelSide, occL, occR }]
        })
  ...
  return (
    <div ref={containerRef} className="h-full w-full select-none overflow-y-auto">
      ...
      >
        ...
        {/* 軸線與事件 */}
        {layout.bands.map(({ track, color, bandTop, bandH, items }) => (
          <g key={track.id}>
            ...
            {items.map(({ ev, kind, shapeL, shapeR, label, labelSide, lane }) => {
